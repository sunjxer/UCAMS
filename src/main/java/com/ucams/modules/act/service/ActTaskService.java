 ...
 */
package com.ucams.modules.act.service;
...
import com.ucams.modules.act.dao.ActDao;
import com.ucams.modules.act.entity.Act;
import com.ucams.modules.act.service.cmd.CreateAndTakeTransitionCmd;
import com.ucams.modules.act.service.cmd.JumpTaskCmd;
import com.ucams.modules.act.service.creator.ChainedActivitiesCreator;
import com.ucams.modules.act.service.creator.MultiInstanceActivityCreator;
import com.ucams.modules.act.service.creator.RuntimeActivityDefinitionEntityIntepreter;
import com.ucams.modules.act.service.creator.SimpleRuntimeActivityDefinitionEntity;
import com.ucams.modules.act.utils.ActUtils;
import com.ucams.modules.act.utils.ProcessDefCache;
import com.ucams.modules.act.utils.ProcessDefUtils;
...
public class ActTaskService extends BaseService {
	 ...
	 * @return
	 */
	public Integer backMessageCount(){
		...
		int count = 0;
		if(EntityUtils.isNotEmpty(todoList)){
			count = count + todoList.size();
		}
		if(EntityUtils.isNotEmpty(toClaimList)){
			count = count + toClaimList.size();
		}
		return count;
	}
...
//					List<HistoricIdentityLink> il = historyService.getHistoricIdentityLinksForProcessInstance(procInsId);
	 ...
	 */
	@Transactional(readOnly = false)
	public HistoricProcessInstance getHisInstance(String procInsId) {
	...
	public String startProcess(String procDefKey, String businessTable, String businessId, String title, Map<String, Object> vars) {
		...
		//给下一节点接收用户发送通知
		this.sendToNextAct(procIns.getId(), null, procIns.getActivityId());
	...
	public void complete(String taskId, String procInsId, String comment, String title, Map<String, Object> vars){
		...
		//给下一节点接收用户发送通知
		if(EntityUtils.isNotEmpty(procInsId)){
			this.sendToNextAct(procInsId, null, null);
		}
...
//		return ProcessDiagramGenerator.generateDiagram(bpmnModel, "png", activeActivityIds);
	 ...
	 * @param taskId    任务ID
	 * @param activityId流程ID
	 * @return
	 * @throws Exception
	 */
	public List<String> getCustomGroupIdentityList(String procInsId,String taskId,String activityId) throws Exception{
		
		List<String> customGroupList = new ArrayList<String>();
		//如果流程ID不为空 且 任务ID为空 则查询当前任务
		if(EntityUtils.isEmpty(taskId) && EntityUtils.isNotEmpty(procInsId)){
			Task task = getCurrentTaskInfo(getProcIns(procInsId));
			if(EntityUtils.isNotEmpty(task)){
				taskId = task.getId();
			}
			
		}
		if(EntityUtils.isNotEmpty(taskId)){
			//activityId为空则获取当前流程   不为空则查询指定activityId流程
			ActivityImpl activity = ProcessCustomService.findActivitiImpl(taskId, activityId);
			//获取用户候选组集合
			...
			for (Expression expression : candidateGroupIdExpressions) {
				String expressionText = expression.getExpressionText();
				customGroupList.add(expressionText);
			}
		}
	 ...
	 * @throws Exception
	 */
	public List<String> getCustomUserIdentityList(String procInsId,String taskId,String activityId) throws Exception{
		List<String> customUserList = new ArrayList<String>();
		//如果流程ID不为空 且 任务ID为空 则查询当前任务
		if(EntityUtils.isEmpty(taskId) && EntityUtils.isNotEmpty(procInsId)){
			Task task = getCurrentTaskInfo(getProcIns(procInsId));
			if(EntityUtils.isNotEmpty(task)){
		...
		if(EntityUtils.isNotEmpty(taskId)){
			...
			for (Expression expression : candidateUserIdExpressions) {
				String expressionText = expression.getExpressionText();
				customUserList.add(expressionText);
			}
		}
		return customUserList;
	}
	
	private void sendToNextAct(String procInsId,String taskId,String activityId){
		//给下一节点接收用户发送通知
		try {
			List<String> groupLists =   getCustomGroupIdentityList(procInsId, taskId, activityId);
			List<String> userLists = getCustomUserIdentityList(procInsId, taskId, activityId);
			if(EntityUtils.isNotEmpty(groupLists) && groupLists.size() > 0){
				SocketIoUtils.sendNewsAsRooms(groupLists);
			}
			if(EntityUtils.isNotEmpty(userLists) && userLists.size() > 0){
				SocketIoUtils.sendNewsAsUsers(userLists);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
