...
public class AmsCheckVideoFileService extends CrudService<AmsPreRptDao, AmsPreRpt> {
	 ...
	 * @return 检查报告
	 */
	public List<String> checkFile(List<AmsFileInfo> fileInfos){
			...
			//检查文件拓展信息
			 ...
			 for(AmsFileInfo file : fileInfos){
				...
				resultsFileMsg = CheckFileUtils.checkObjFieldIsNull(file); 				 
				 ...
				 //文件信息检查报告
				 orgenRptList.addAll(this.orgenRpt(resultsFileMsg, file.getFileName()));
				 //---------------------------------------检查物理文件------------------------------------//
				 orgenRptList.addAll(this.checkElectronicFile(file));
			}
	 ...
	 * @throws IOException 
	 */
	private List<String> checkElectronicFile(AmsFileInfo file) throws IOException{
		 List<String> resultsFileMsg = new ArrayList<String>();
		 //组织文件地址
...
//					}
		 ...
		 return this.orgenRpt(resultsFileMsg, file.getFileName());
...
//				 containers.put(unit.getId(), unit.getUnitProjectName());  //将单位工程信息存入map容器
	 ...
	 * @return
	 */
	private List<String> orgenRpt(List<String> results, String fileName) {
