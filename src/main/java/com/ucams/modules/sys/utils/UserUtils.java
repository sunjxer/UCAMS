 ...
 */
package com.ucams.modules.sys.utils;
...
import com.ucams.modules.sys.dao.AreaDao;
...
public class UserUtils {
	...
	public static final String USER_OF_ROLE_ = "userOfRole_id";
	 ...
	 * 获取当前用户所属角色的英文名
	 * @param id
	 * @return 取不到返回null 工作流
	 */
	public static List<String> getUserOfRole(){
		List<String>  enamesList = (List<String>)CacheUtils.get(USER_CACHE, USER_OF_ROLE_ + getUser().getId());
		if (EntityUtils.isEmpty(enamesList)){
			List<Role> roles = roleDao.findList(new Role(getUser()));
			enamesList = new ArrayList<String>();
			for(Role role : roles){
				enamesList.add(role.getEnname());
			}
			CacheUtils.put(USER_CACHE, USER_OF_ROLE_ + getUser().getId(), enamesList);
		}
		return enamesList;
	}
