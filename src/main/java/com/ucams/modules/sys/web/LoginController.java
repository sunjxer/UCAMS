...
import com.ucams.common.servlet.ValidateCodeServlet;
import com.ucams.common.utils.CacheUtils;
import com.ucams.common.utils.CookieUtils;
...
import com.ucams.modules.sys.security.FormAuthenticationFilter;
import com.ucams.modules.sys.security.SystemAuthorizingRealm.Principal;
...
//		try {
//			byte[] bytes = com.ucams.common.utils.FileUtils.readFileToByteArray(
//					com.ucams.common.utils.FileUtils.getFile("c:\\sxt.dmp"));
...
//		System.out.println("==========================b");
		...
		//检测责任主体是否填写信息
		if(this.checkIsFirstLogin(principal)){
			return "modules/sys/AddResBodyInfo";
		}
		return "modules/sys/sysIndex";
	}
	...
	public boolean checkIsFirstLogin(Principal principal){
		if (principal!=null){
			User user = UserUtils.get(principal.getId());
			for(Role role : user.getRoleList()){
				if (	user != null 
						&& ! UcamsConstant.AMS_RESBODY_TYPE_NONE.equals(role.getRoleType())
						&& ! UcamsConstant.AMS_RESBODY_TYPE_JSDY.equals(role.getRoleType())){
					if(EntityUtils.isEmpty(amsUnitDetailinfoService.getByUnitId(new AmsUnitDetailinfo(role)))){
						return true;
					}				
				}
			}
