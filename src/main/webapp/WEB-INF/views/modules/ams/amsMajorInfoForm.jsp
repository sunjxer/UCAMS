...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/ams/amsUnitProInfo/majorForm?id=${amsUnitProInfo.id}">专业记载信息</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="amsUnitProInfo" action="${ctx}/ams/amsUnitProInfo/majorSave" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<sys:message content="${message}"/>		
		<div class="control-group">
			<label class="control-label">单位工程:</label>
			<div class="controls">
				<span class="help-inline">${amsUnitProInfo.unitProjectName}</span>
			</div>
		</div>
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<c:if  test="${amsDesExtend.showType==\"number\"}">
				 <input type="number" id="amsDesExtendList[${status.index}]_settings" name="amsDesExtendList[${status.index}].settings" maxlength="10"  class="input-xxlarge" class="input-small${amsDesExtend.isNull==1?'required':''}">${amsDesExtend.settings}</> 		
				</c:if>
